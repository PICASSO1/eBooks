...
/*  gnlh = nlmsg_data(nlh); */
    ...
    if (tb[NL80211_ATTR_GENERATION]) {
        unsigned int generation = nla_get_u32(tb[NL80211_ATTR_GENERATION]);
        printf("NL80211_ATTR_GENERATION (%d) : %u\n", NL80211_ATTR_GENERATION, generation);
    }

    if (tb[NL80211_ATTR_4ADDR]) {
        unsigned char use_4addr = nla_get_u8(tb[NL80211_ATTR_4ADDR]);
        printf("NL80211_ATTR_4ADDR (%d) : %u\n", NL80211_ATTR_4ADDR, use_4addr);
    }

    if (tb[NL80211_ATTR_WIPHY_FREQ]) {
        unsigned int freq = nla_get_u32(tb[NL80211_ATTR_WIPHY_FREQ]);
        printf("NL80211_ATTR_WIPHY_FREQ (%d) : %u MHz\n", NL80211_ATTR_WIPHY_FREQ, freq);
    }

    if (tb[NL80211_ATTR_WIPHY_CHANNEL_TYPE]) {
        unsigned int chan_type = nla_get_u32(tb[NL80211_ATTR_WIPHY_CHANNEL_TYPE]);
    ...
    if (tb[NL80211_ATTR_CHANNEL_WIDTH]) {
        unsigned int width = nla_get_u32(tb[NL80211_ATTR_CHANNEL_WIDTH]);
    ...
    if (tb[NL80211_ATTR_WIPHY_TX_POWER_LEVEL]) {
        unsigned int tx_power = nla_get_u32(tb[NL80211_ATTR_WIPHY_TX_POWER_LEVEL]);
    ...
    if (bss[NL80211_BSS_SIGNAL_UNSPEC]) {
        unsigned char signal_unspec = nla_get_u8(bss[NL80211_BSS_SIGNAL_UNSPEC]);
    ...
    if (bss[NL80211_BSS_CHAIN_SIGNAL]) {
        ...
        nla_for_each_nested(chain, bss[NL80211_BSS_CHAIN_SIGNAL], rem) {
            ...
            char signal = (char)nla_get_u8(chain);
