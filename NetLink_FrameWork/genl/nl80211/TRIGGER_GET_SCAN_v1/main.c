 ...
 *
 * Version: 1.0.0.build051026
 *
 * Date: 2026 / 05 / 10
 *
 * Description: 
 *
 * 01. 因為在 main.c 當中，標頭檔的順序是：main.h --> trigger_scan.h ，對 main.c 而言並不會有什麼問題；但在 "trigger_scan.h" 並不知道 main.h 裡有甚麼東西；
 *     所以在 trigger_scan.h 當中有「向前宣告」 `struct nl80211_state; ` ---- 有一個資料型態叫做 struct nl80211_state; 而真正的宣告則是在 main.h。
 * 
 * 02. 針對巢狀屬性 (Nested Attribute)的資料，只列印出長度即可 (Len: %d)；其中 NL80211_BSS_INFORMATION_ELEMENTS 和 NL80211_BSS_BEACON_IES 的內容非常繁瑣，目前不需要，有空再分析。
 * 
 * 03. 在 trigger_scan.c 當中有一段 `NL80211_ATTR_SCAN_FREQUENCIES` 註解掉的程式碼：
 *     在 Scanning 的過程中，假如 User Process 沒有指定頻道 (Channel)，那 WLAN Driver 就會使用自行預設的頻道來進行掃描。
 *     所以在商業化的產品中，例如 RFScand 就必須使用 for loop 分別對 2.4GHz, 5GHz, 6GHz 所需要掃描的頻道進行設定。
 * 
 * 04. 這一支 Active Scanning 是最簡單的架構，從頭到尾就只用了２個函式：
 *     nl80211_trigger_scan(); 向 WLAN Driver 發送 scan 的請求；
 *     nl80211_get_scan();     向 WLAN Driver 接收 scan 的結果。
 *     優點：架構簡單、好 debug、適合初步瞭解 Wireless Scannin 的行為、好抓封包、callback 單純；
 *     缺點：scanning 時間不可控制、在實際的產品應用上並不會這樣子寫、Scan Aborted 不好處理。
 *
(*)?*/
...
{
    size_t i = 0, num = sizeof(nl80211_bss_scan_width_type_map) / sizeof(struct nl80211_bss_scan_width_type);
