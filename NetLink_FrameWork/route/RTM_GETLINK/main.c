 ...
 *
 * Version: 1.0.0.build033126
 *
 * Date: 2026 / 03 / 31
 *
 * Description: `nlh->nlmsg_type = RTM_GETLINK;`
 *
 * 1. 
 *
(*)?*/
...
char **envp;
{
	...
	nlh->nlmsg_type = RTM_GETLINK;                                             /* Message type: Get network interface info (18) */
	nlh->nlmsg_flags = NLM_F_REQUEST | NLM_F_ACK | NLM_F_DUMP;
	nlh->nlmsg_seq = ++seq;
	nlh->nlmsg_pid = getpid();

	/* NLMSG_DATA() 巨集的用意是將 nlh 原本的記憶體位址，加上 Netlink Header Length 之後，回傳給 ifi */
	ifi = (struct ifinfomsg *)NLMSG_DATA(nlh);
	memset(ifi, 0x00, sizeof(struct ifinfomsg));
	/* ifinfomsg structure 有很多個成員，並非每一個都需要設定；視 nlh->nlmsg_flags 來決定 */
	...
	while (1) {
		...
		for (nlh = (struct nlmsghdr *)buffer; NLMSG_OK(nlh, len); nlh = NLMSG_NEXT(nlh, len)) {
			...
			parse_interface_family(ifi->ifi_family);

			/* print device type */
			parse_device_type(ifi->ifi_type);

			/* print interface index */
			printf("Interface Index: %d \n", ifi->ifi_index);

			/* print device flags */
			parse_device_flags(ifi->ifi_flags);
