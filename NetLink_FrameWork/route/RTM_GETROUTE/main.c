 ...
 * Date: 2026 / 04 / 04
 *
 * Description: `nlh->nlmsg_type = RTM_GETROUTE;`
 *
 * 1. 
 *
(*)?*/
...
char **envp;
{
	...
	struct rtmsg *rt = (struct rtmsg *)NULL;
	char buffer[BUFSIZE];
	...
	nlh->nlmsg_type = RTM_GETROUTE;                                             /* Message type: Get Network Route (26) */
	nlh->nlmsg_flags = NLM_F_REQUEST | NLM_F_ACK | NLM_F_DUMP;
	nlh->nlmsg_seq = ++seq;
	nlh->nlmsg_pid = getpid();

	/* NLMSG_DATA() 巨集的用意是將 nlh 原本的記憶體位址，加上 Netlink Header Length 之後，回傳給 rt */
	rt = (struct rtmsg *)NLMSG_DATA(nlh);
	memset(rt, 0x00, sizeof(struct rtmsg));
	/* rtmsg structure 有很多個成員，並非每一個都需要設定；視 nlh->nlmsg_flags 來決定 */
	...
	while (1) {
		...
		for (nlh = (struct nlmsghdr *)buffer; NLMSG_OK(nlh, len); nlh = NLMSG_NEXT(nlh, len)) {
			...
			/* print addres type */
			parse_address_type(rt->rtm_family);

			/* print Length of Destination */
			...
			/* print ToS filter */
			printf("TOS Filter: 0x%02X \n", rt->rtm_tos);

			/* print Routing Table ID */
			printf("Routing Table ID: %u \n", rt->rtm_table);

			/* print Routing Protocol */
			parse_routing_protocol(rt->rtm_protocol);

			/* print Route Origin */
			parse_route_origin(rt->rtm_scope);
			
			/* print Route Type */
			parse_route_type(rt->rtm_type);
			
			/* print route flags */
			printf("Routing Flags: 0x%08X \n", rt->rtm_flags);

			/* RTA_OK() 判斷是否已經讀取到最後乙筆 attribute；否則就繼續讀取下乙筆屬性 RTA_NEXT() */
			for (; RTA_OK(attr, attrlen); attr = RTA_NEXT(attr, attrlen))
				parse_rta_attributes(attr, rt->rtm_family);
